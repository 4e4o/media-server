...
{
	return mov_writer_write_l(writer,track,data,bytes,pts,dts,flags,0);
}

int mov_writer_write_l(struct mov_writer_t* writer, int track, const void* data, size_t bytes, int64_t pts, int64_t dts, int flags , int add_nalu_size)
{
    struct mov_t *mov;
    ...
    if (!add_nalu_size) {
        ...
        mov_buffer_write(&mov->io, nalu_size_buf, 4);
        mov_buffer_write(&mov->io, data, nalu_size);
    }
